...
export const generateForgetPasswordLink: RequestHandler = async (req, res) => {
  ...
  // Generate the link
  // https://yourapp.com/reset-password?token=htjakjfja235kdas&userId=63729jajhvruh764
...
export const updateProfile: RequestHandler = async (req: RequestWithFiles, res) => {
  ...
  if(avatar){
    ...
    // upload new avatar file
      ...
      const {secure_url, url, public_id} = await cloudinary.uploader.upload(avatar.filepath, {
        width: 300,
        height: 300,
        crop: "thumb",
        gravity: "face"
...
export const logOut: RequestHandler = async (req, res)=>{
  ...
  // Application from one media-device, Log out from all media
  ...
  // else user.tokens = user.tokens.filter((tkn) =>{
  //   if(tkn === token) return false;
  //   else return true;
  // })
