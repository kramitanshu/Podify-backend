...
export const createPlaylist: RequestHandler = async (
  ...
  /*
 -- While creating playlist there can be request with new playlist name and the audio that user wants to store inside the playlist
OR
 -- User just want to create an empty playlist
*/
...
export const updatePlaylist: RequestHandler = async (
  ...
  if (!playlist) return res.status(404).json({ error: "Playlist not found1" });

  if (item) {
    const audio = await Audio.findById(item);
    if (!audio) return res.status(404).json({ error: "Audio not found1" });
...
// '/playlist?playlistId=jkajfjijiat7492&resId=faiigacy&all=yes'
